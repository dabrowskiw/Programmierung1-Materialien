...
Mit diesem Wissen bewaffnet ist es schon mal möglich, den ersten Schritt zum Zählen der Ecken zu machen: Eine neue Variable `numberOfCorners` anlegen (diese Schreibweise, in der bei langen Variablennamen der Anfangsbuchstabe von jedem Wort groß geschrieben wird, wird übrigens als "camel case" bezeichnet, da die Großschreibung an die Höcker eines Kamels erinnert), den Anfangswert auf 0 setzen, und dann in jedem Schritt eine Sekunde lang den Wert der Variable ausgeben lassen:
